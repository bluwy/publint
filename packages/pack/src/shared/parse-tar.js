...
export function parseTar(data) {
  ...
  while (offset < data.byteLength) {
    ...
    const type = read(data, decoder, offset + 156, 1)

    // Skip empty blocks at end
    if (type === '\0') break

    // Get file size from header (from offset 124, 12 bytes)
    const size = parseInt(read(data, decoder, offset + 124, 12), 8)
    ...
    if (type === '0') {
      ...
      const name = read(data, decoder, offset, 100).split('\0', 1)[0]

      // Get file content from header (from offset 512, `size` bytes)
      const data = new Uint8Array(data, offset + 512, size)
...
export function getFilesRootDir(files) {
  return files.length ? files[0].split('/')[0] : 'package'
}
